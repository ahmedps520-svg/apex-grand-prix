...
### Push 36: The daily challenge (v0.36.0)
- **The challenge** (`content/daily.ts`: `dailyChallenge(date)` draws a circuit, a car, a time of day and a weather from the UTC date's seed, dry days twice as likely; `dailyKey` YYYY-MM-DD): a *Daily Challenge* tile across the bottom of the main menu names the day's circuit, car and conditions and today's best; it starts a time trial on them (sim handling, the day still, the weather fixed; `SessionSetup.daily`, `SessionConfig.daily`).
...
### Push 38: Career (v0.38.0)
- **The ladder** (`content/career.ts`: `CAREER_TIERS`, six series from the Street Series (3 races, 2 laps, 7 rivals, easy, top 3 to move up) through the Dune Cup, the Touring Championship, the GT Challenge and the Endurance Prototypes to Formula Apex (6 races, 4 laps, 11 rivals, hard, only the title moves you on); `advanceCareer` keeps the latest attempt at a tier and moves the tier up when the finish earns it; `isCareerSave`): a Career button on the championship screen opens the Career screen: the tier's series, a car of its class to choose, *Start the …* with the season's terms, and the ladder with each series done (the finish), current or locked (what it takes above).
- **A season** is a championship (`Championship.career` = the tier; mixed cars of the class, the tier's rivals, laps and difficulty, random circuits) run through the usual rounds and standings; *Continue season* and *Standings* on the Career screen while one is under way, the standings' note saying what the finish means; when the last round is scored the career moves up with a toast (Promoted to the …!), or says the series can be run again. *Restart career* forgets everything. The career (`apex-gp.career`) keeps the tier and every season's result.
- **Tests:** the tiers are real classes with seasons that fit the calendar and a promotion place within the field; the career moves up on a finish good enough, keeps the latest attempt, ignores a season at a tier already passed and completes at the top; a stored career is checked before it is trusted. 405 unit tests.
- **Honest limits:** the career's season lives in the one championship slot: starting a plain championship replaces it (the tier is kept, the season restarts); no prize money, contracts or car upgrades; the same rivals' names throughout.

### Push 39: Tyre wear (v0.39.0)
- **The model** (`Wheel.wear`, `Car.wearRate`, `wearGrip`): the tread goes with the sliding done under load at speed (combined slip × load share × speed, at the rate set), and the grip falls away with it, faster towards the end (18 % gone at a bare tyre); a restart or `freshTyres` brings a new set. Every car in the race wears the same way, and the AI's speed plan follows the grip left (its pace scale by the root of it), so they slow as you do.
- **The option** (`SessionSetup.tyreWear` off / normal / fast on the race setup and the championship screen; `SessionConfig.tyreWear` 0 / 1 / 2.5): off by default. A tread panel on the speedo (four bars, FL FR RL RR; amber past half, red past 80 %) shows in races with wear, the snapshot carrying each wheel's wear (`W.WEAR`); the engineer says when the tyres are half worn and when they are going off.
- **Tests:** an AI lap of the 2.3 km test loop at the normal rate takes a few per cent of the tread and none when off, the fast rate more than twice as much, and new tyres are new; a session sets the rate for every car and a restart brings new tyres; the grip falls with the tread, faster towards the end. 408 unit tests.
- **Honest limits:** no pit stops yet (the next push): with wear on, a long race is run on one set; no tyre temperatures or compounds; the rate is a first calibration from the AI's laps.

### Push 40: Pit stops (v0.40.0)
- **The lane** (`shared/pitLane.ts`, for the simulation and the scenery alike): every circuit has one beside its start straight: it leaves the track 300 m before the line, runs 4.5 m beyond the left edge past the box 90 m before the line, and rejoins 80 m after it, moving across over 70 m at each end; the scenery draws the strip and a wall along its outer side (`TrackScene.buildPitLane`).
- **The stop** (`sim/race/Pits.ts`, `Car.rideRails`, `Car.onRails`): a car that means to box goes onto rails at the entry (its own line blended into the lane's), brakes to the lane's 80 km/h limit and to a stop at the box, waits 4 s for new tyres and its damage mended, pulls away and is handed back on the track at the exit; on rails the physics and the contacts leave it alone, the wheels roll with the speed, and the lap counts as it passes the line in the lane. The AI boxes on its own once its tyres are more than 60 % gone with two laps or more to go; the player asks (`pit` command). A restart empties the lane.
- **The ask** (`SessionConfig.pitStops`, open in races with tyre wear on): the T key (rebindable, *pit*), or the quick menu's *Pit stop: Stay out / Box this lap*; the HUD says BOX THIS LAP, PIT LANE, PIT STOP · s and PIT EXIT (`RaceHud.updatePit`), the engineer calls "Box, box" and sends the car off with new tyres.
- **Tests:** the lane leaves the track, runs beside it and rejoins; a car armed before the entry goes in, stops at the box in the lane with its tyres still worn, gets new ones after the stop's 4 s, and is handed back just after the line rolling at the limit, all inside a minute; the AI boxes only on worn tyres with laps to go, never the player's car, never on fresh tyres. 411 unit tests.
- **Honest limits:** the pit lane runs on the run-off, on rails: no driving it yourself, no speeding penalty, and the wall is scenery; one box for everyone (cars in the lane at once share it without a queue); no pit crew to see; the AI's strategy is one threshold, not a plan.

### Push 41: Race rules (v0.41.0)
- **Track limits** (`RaceDirector.enforce`, `status.rules`; *Race rules: Track limits and flags / Off* on the race setup and the championship screen, on by default): an excursion with all four wheels off the road and the kerbs is a cut when it is 12 to 90 m long and never drops below 10 m/s (a crawl across the grass is a crash, its own punishment): a warning, and every third warning a 3 s penalty, added to the time at the flag (`finishTime`, so the order at the flag is the order after penalties). The same for every car, AI included. The HUD shows TRACK LIMITS · WARNING n and +3 s PENALTY as they land, the results card a "· 3 s pen." beside the time, and the engineer says so.
- **Flags** (`CarRaceState.flag`, `status.yellow`): a car stopped on the track for two seconds puts its sector under a yellow: cars in that sector are shown YELLOW FLAG · SECTOR n and the AI eases to 80 % of its plan there; a car a lap or more up within 30 m behind another shows that car a blue: BLUE FLAG on the HUD, the AI lifting to 90 % to let it by. The engineer calls both.
- **Tests:** a quick 25 m over the grass and back is a warning, three are a penalty, a slow crawl across the grass is not, and the car first over the line loses to the one behind by its penalty; a stopped car's sector goes yellow for the cars in it (not the stopped car) and clears when it moves, and a car a lap up close behind shows the other a blue; a session's rules follow the config. 414 unit tests.
- **Honest limits:** no safety car, no stop-and-go, no drive-through: penalties are seconds at the flag; the cut rule is by surface and distance, not by advantage gained; blue-flagged AI lifts but does not move aside; the yellow slows the AI in the sector only.

### Push 42: Post-processing (v0.42.0)
- **The pipeline** (`render/Effects.ts`: three's `RenderPipeline` over a scene pass with an MRT): the frame rendered to a texture with its emissive light kept apart, a bloom on that light alone (the brake lights, the headlights and strobes, the lit windows and signs, the helicopter's beacon: strength 0.4, radius 0.3, threshold 0.25), and at the full level ambient occlusion (GTAO from the pass's depth and normals, at half resolution) darkening the corners the light can't reach, then tone-mapped to the screen as before. The sunlit road and the sky never glow, so the day stays clean: a bloom on the frame's brightness hazed the whole frame until its threshold passed the sky's. Photo mode keeps its own pipeline.
- **The setting** (`Settings.effects`: *Effects: By detail level / Off / Bloom / Bloom and ambient occlusion* on the Graphics tab): by detail, low is off (so the tests and the weakest devices see the plain render), medium a bloom, high the full set; the pipeline is rebuilt on every session start and when the setting changes, and falls back to the plain render with a console warning if the renderer can't set it up.
- **Tests:** the settings know the effects levels and default to auto. 414 unit tests.
- **Honest limits:** no screen-space reflections yet (the wet road mirrors the sky through its material alone); the sun's disc and the glare on a wet road don't bloom, only emissive light does; the bloom's numbers are set by eye in software rendering; ambient occlusion's cost on an iPad is untested (it stays off there by detail).

### Push 43: Safety car (v0.43.0)
- **The car** (`sim/race/SafetyCar.ts`: its own `Car` and `AiDriver` with a speed limit, a slot after the racers in the snapshot): with the race rules on (never in an elimination race or a qualifying, and only in races of three laps or more) a GT in silver with an amber light bar waits out of sight in the pit box. A yellow flag that stands for 5 s brings it out (not in the last two laps, nor within 60 s of a restart): it joins the track 70 m ahead of the leader at 80 % of its speed, hazards flashing, and drives the racing line at 24 m/s (86 km/h). The AI behind it caps its speed at the same 24 m/s and holds station (it never pulls out to pass); the player is shown SAFETY CAR · NO OVERTAKING and each car passed under it (the safety car included, a pitting car excepted) is a 3 s penalty at the flag, added like a track-limits one. Once it has crossed the line the board says IN THIS LAP, and at the pit entry at the end of that lap it goes onto the pit rails, the race is green the moment it does (GREEN FLAG · GO, the limits lifted), and it rides the lane to its box and vanishes there. A finish under it parks it; a restart empties its count.
- **Around it:** `RaceFlag` gains `safety` (over a yellow or a blue for every car on the track), `RaceStatus.safetyCar` the phase and laps, `CarRaceState.penaltyFor` what the last penalty was for (the HUD's notice and the engineer's call say which); the engineer calls it out, the lap it comes in, the green, and the penalty; its minimap dot is hidden while it waits (and an eliminated car's with it), replays cycle through the racers only; the car view grows a `lightBar` (`police` red and blue with the siren, `safety` amber with the hazards); the minimap shows it amber; the rules choice reads *Track limits, flags and safety car*; `window.__apex.safetyCar()` and the `safetyCar` sim command bring it out at once.
- **Tests:** it waits in its box with the rules and has no place in an elimination race or a qualifying; it comes out ahead of the leader once a yellow has stood, the field is shown the board and the AI's limit and hold are set, and it keeps to its speed; a pass by the player under it is a penalty for the safety car; with no laps to lead it goes in at the entry, the race goes green with the limits lifted, it rides to its box out of sight, and a restart empties the count. 418 unit tests.
- **Honest limits:** it joins the track by appearing ahead of the leader (no drive out of the pit exit), lapped cars ahead of it stay there (no wave-by), the leader has no restart to control (green as the safety car enters the lane), the AI never boxes more cleverly under it, and the player's penalty is by position on the track (a pass off the track counts the same).

### Push 44: Graphics presets, realistic asphalt, rubber and ray-traced reflections (v0.44.0)
- **Presets** (`app/graphics.ts`, *Preset* at the top of the Graphics tab): Auto picks by the device (low, medium or high), Low / Medium / High / Ultra write their values into the choices below, and changing any of those makes it Custom. Low is the plain render with the renderer's MSAA, standard roads and skid marks; Medium adds the bloom, the detailed asphalt (512 maps) and FXAA; High ambient occlusion, a colour grade and vignette, soft shadows, 1024 asphalt maps and SMAA; Ultra the screen-space ray-traced reflections and a 4096 shadow map on top. New choices: *Reflections* (Off / Ray-traced, screen space), *Anti-aliasing* (Off / FXAA / SMAA), *Shadows* (Low 1024 / Medium 2048 / High soft / Ultra soft 4096: the map size and the PCF radius, applied live), *Road surfaces* (Standard / Detailed asphalt and rubber) and *Skid marks*.
- **Asphalt** (`render/asphalt.ts`): baked once per size into tileable maps, 5–25 mm stones of aggregate (grey to buff, a few proud) in a dark weathered bitumen binder with its own grain and a soft tonal wander; a normal map from the stones' domes and the binder's grain; roughness dull on the binder, polished on the stones. The road reads them twice (the second look-up turned and scaled, blended in patches, so the 7 m tile never repeats), with a tonal wander over 60 m and the odd darker repair; in the wet it goes dark and glossy with standing water in the dips, the water lying flat over the stones. The city's streets get the same asphalt in world space.
- **Rubber** (`bakeRubber`): the racing line (the one the AI drives, worked out once per circuit on the main thread, about 0.15 s) laid into a map along the lap and across the road: a band where the cars run, darkest in the two wheel tracks 0.8 m either side of the line, heavier where they brake hard, smoothed over 12 m, streaked along the direction of travel in the shader, with launch marks off every grid box. Rubber darkens the asphalt, fills its texture and polishes it.
- **Skid marks** (`render/SkidMarks.ts`): every wheel near the camera that slides, locks or spins up on tarmac or a kerb lays a dark strip the width of its tyre from its contact patch, fading in and out with how hard it slides; strips join without gaps, a jump starts a new mark, and a ring of 6000 strips is reused oldest first. A clean road each session.
- **Reflections** (`render/Effects.ts`): the scene pass writes a reflection request per surface into its own MRT channel (strength and roughness: the wet road by its wetness and puddles, the paint by its clear coat and metalness, the glass), and three's SSR node traces each requesting pixel's mirror ray through the depth buffer at half resolution (60 m reach) and adds what it finds; the rest of the frame is left alone. The requests are attached to the materials only while a pipeline traces (a material with an MRT output of its own draws nothing into a target without that MRT, and the plain render goes through the renderer's own frame buffer); photo mode's pass carries an MRT too. The pipeline also gained SMAA (before the tone mapping) or FXAA (after), and the grade.
- **The city's light:** its streets lie in the towers' shade, where the open sky's blue fill turned the tarmac navy; the fill now takes a grey bounce off the concrete (70 %), the sky's share of the fill and reflections is cut to 40 % and the hemisphere made up by 40 %, so shaded asphalt reads dark grey.
- **Tests:** the presets' ladder, auto by device, a preset written into the choices and custom kept, the settings saved and repaired; the effects key; the asphalt deterministic, grey with polished stones and dull binder, bumps facing up, no seam at the wrap; the rubber on the line and darkest in the wheel tracks, heavier in the braking zone, launch marks on the grid; skid intensity from sliding, locking and wheelspin and never from grip, grass or air; joined strips, a fresh start after a gap, the ring reused and cleared; the reflection requests attached only while tracing. 433 unit tests.
- **Honest limits:** browsers give pages no hardware ray tracing, so the reflections are screen-space: only what is on screen can be reflected, and they fade at the frame's edges; the rubbered line is the AI's default line, not where you have driven; skid marks come only from cars within 120 m of the camera and don't wash off in the rain; the detailed asphalt costs ten texture reads per road pixel; Ultra's cost on an iPad is untested (Auto never picks it).
