import { hourOf, sunPathAt, type Conditions } from '../conditions';
...
export function citySunAzimuthAt(hour: number): number {
  return sunPathAt(hour, CITY_SUN_ELEVATION, CITY_SUN_AZIMUTH).azimuth;
}
...
export function citySunAt(hour: number): { elevation: number; azimuth: number } {
  return sunPathAt(hour, CITY_SUN_ELEVATION, CITY_SUN_AZIMUTH);
}
